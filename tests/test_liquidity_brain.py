...
def test_combined_spread_is_sum_of_both_legs_ask_minus_bid(brain):
    reading = brain.analyze(ce_bid=100.0, ce_ask=101.0, pe_bid=50.0, pe_ask=50.5, context=TEST_CONTEXT)
...
def test_per_leg_spread_pct_uses_own_leg_mid(brain):
    reading = brain.analyze(ce_bid=100.0, ce_ask=102.0, pe_bid=50.0, pe_ask=50.0, context=TEST_CONTEXT)
...
def test_tight_spread_classifies_tight(brain):
    reading = brain.analyze(ce_bid=100.0, ce_ask=100.1, pe_bid=80.0, pe_ask=80.1, context=TEST_CONTEXT)
    assert reading.tightness is SpreadTightness.TIGHT


def test_wide_spread_classifies_wide(brain):
    reading = brain.analyze(ce_bid=100.0, ce_ask=106.0, pe_bid=80.0, pe_ask=85.0, context=TEST_CONTEXT)
...
def test_moderate_spread_classifies_normal(brain):
    ...
    reading = brain.analyze(ce_bid=100.0, ce_ask=101.0, pe_bid=80.0, pe_ask=80.8, context=TEST_CONTEXT)
...
def test_nonpositive_quote_field_returns_unknown(brain, field):
    ...
    kwargs[field] = 0.0
    reading = brain.analyze(**kwargs, context=TEST_CONTEXT)
...
def test_crossed_market_ce_returns_unknown(brain):
    reading = brain.analyze(ce_bid=101.0, ce_ask=100.0, pe_bid=80.0, pe_ask=80.5, context=TEST_CONTEXT)
    assert reading.tightness is SpreadTightness.UNKNOWN
    assert "crossed_market" in reading.reason


def test_crossed_market_pe_returns_unknown(brain):
    reading = brain.analyze(ce_bid=100.0, ce_ask=101.0, pe_bid=80.5, pe_ask=80.0, context=TEST_CONTEXT)
    assert reading.tightness is SpreadTightness.UNKNOWN
    assert "crossed_market" in reading.reason
...
def test_render_handles_unknown_reading_without_raising(brain):
    reading = brain.analyze(ce_bid=0.0, ce_ask=100.0, pe_bid=80.0, pe_ask=80.5, context=TEST_CONTEXT)
...
def test_real_quote_2026_07_20_24250_strike_classifies_tight():
    ...
    reading = brain.analyze(ce_bid=82.4, ce_ask=82.6, pe_bid=68.0, pe_ask=68.05, context=TEST_CONTEXT)
