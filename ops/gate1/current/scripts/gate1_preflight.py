...
if session_id:
    ...
    if check("universe built", universe_present or not expect_universe,
        ...
        # ---- THE PAIR IS ONE PIECE OF EVIDENCE -------------------------
        #
        # universe.json and universe_manifest.json are validated together or
        # not at all. Three gaps this replaces, all found by audit:
        #
        #   1. THE HASH CHECK WAS CONDITIONAL. `if man.get("universe_sha256")`
        #      meant a manifest lacking the key skipped the comparison
        #      ENTIRELY and the run passed. A validation that only runs when
        #      its input happens to be present is not a validation.
        #   2. "CANNOT BE READ" WAS NOT GRADED. json.loads had no try, so a
        #      corrupt manifest raised an uncaught JSONDecodeError and killed
        #      the preflight with a traceback instead of a named refusal.
        #      Fail-closed by accident is not the same as fail-closed.
        #   3. THE PAIR WAS NEVER MATCHED BY NAME. The manifest carries
        #      `universe_file`, but nothing checked it described the file
        #      actually being hashed -- so pairing rested on the hash alone,
        #      which collapses completely under gap 1.
        #
        # A missing symbol hash cannot reconstruct a lost universe.json. It
        # verifies a candidate symbol set you already hold. The pair still
        # requires BOTH files; that is why every branch below refuses rather
        # than degrading to a partial check.
        uni = {}
        _uni_valid = False
        try:
            ...
            uni = {}
            check("universe.json parses", False,
                  f"{uni_path} could not be read as JSON "
                  f"({type(exc).__name__}: {exc})")

        man = {}
        _man_valid = False
        if not man_path.exists():
            check("universe manifest written", False,
                  f"{man_path} is ABSENT -- the universe and its manifest are one "
                  f"evidence pair and neither is valid alone")
        else:
            try:
                ...
                man = {}
                check("universe manifest parses", False,
                      f"{man_path} could not be read as JSON "
                      f"({type(exc).__name__}: {exc})")

        if _uni_valid and _man_valid:
            _pair_ok = True

            # -- schema, explicitly. An unknown version is a refusal, never a
            #    silent best-effort read of a shape nobody has seen.
            _schema = str(man.get("schema") or "")
            if _schema == "gate1.universe_manifest/1":
                _required = ("universe_file", "universe_sha256")
            elif _schema == "gate1.universe_manifest/2":
                _required = ("universe_file", "universe_sha256",
                             "symbols_sha256", "symbols_sha256_method")
            else:
                _required = ()
                _pair_ok = check("manifest schema is supported", False,
                                 f"schema {_schema!r} is not a version this preflight "
                                 f"knows how to validate (supported: "
                                 f"gate1.universe_manifest/1, /2)")
            if _required:
                check(f"manifest schema is supported", True, _schema)
                _absent = [k for k in _required if not man.get(k)]
                if _absent:
                    _pair_ok = check("manifest carries every required field", False,
                                     f"{_schema} requires {list(_required)}; missing or "
                                     f"empty: {_absent}. A missing hash is a FAILURE, "
                                     f"never a skipped check.")
                else:
                    check("manifest carries every required field", True,
                          ", ".join(_required))

            # -- the manifest must describe THIS file, by name.
            if _pair_ok:
                _declared = str(man.get("universe_file") or "")
                try:
                    _same = Path(_declared).resolve() == uni_path.resolve()
                except Exception:  # noqa: BLE001
                    _same = False
                _pair_ok = check("manifest names the universe being validated", _same,
                                 f"manifest declares {_declared!r}; validating "
                                 f"{uni_path}") and _pair_ok

            # -- content hash, UNCONDITIONAL.
            if _pair_ok:
                _actual = hashlib.sha256(uni_path.read_bytes()).hexdigest()
                check("universe matches its manifest hash",
                      man["universe_sha256"] == _actual,
                      "unchanged since it was built"
                      if man["universe_sha256"] == _actual else
                      "universe.json CHANGED after the manifest was written -- "
                      "it is no longer the artifact that was planned")

                # -- v2: the symbol SET, independent of file formatting.
                if _schema == "gate1.universe_manifest/2":
                    _syms = [i.get("symbol") for i in uni.get("instruments", [])]
                    if not all(_syms):
                        check("every instrument carries a symbol", False,
                              "an instrument has no symbol; the subscribed set "
                              "cannot be identified honestly")
                    elif len(set(_syms)) != len(_syms):
                        check("symbols are unique", False,
                              f"{len(_syms) - len(set(_syms))} duplicate symbol(s) -- "
                              f"the subscribed set is not what the count claims")
                    else:
                        _sh = hashlib.sha256(
                            "\n".join(sorted(_syms)).encode("utf-8")).hexdigest()
                        check("symbol set matches its manifest hash",
                              man["symbols_sha256"] == _sh,
                              f"{len(_syms)} symbols verified against "
                              f"symbols_sha256"
                              if man["symbols_sha256"] == _sh else
                              "the symbol SET changed since the manifest was "
                              "written, even if the file bytes did not")
        ...
        # The hash comparison now lives in the pair block above, where it is
        # unconditional. It used to sit here behind `if man.get(...)`.
