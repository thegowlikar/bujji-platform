...
def _symbols_sha256(instruments) -> str:
    """Stable identity of the symbol SET, independent of file formatting.

    Sorted so that a reordering of otherwise identical membership does not
    read as a different universe. Raises on a missing symbol rather than
    hashing a silent gap: an instrument with no symbol is not something to
    quietly exclude from the identity of what was subscribed.
    """
    import hashlib as _h
    symbols = []
    for i in instruments:
        sym = i.get("symbol") if isinstance(i, dict) else getattr(i, "symbol", None)
        if not sym:
            raise SystemExit("instrument without a symbol: cannot hash the "
                             "symbol list honestly")
        symbols.append(sym)
    if len(set(symbols)) != len(symbols):
        raise SystemExit(f"duplicate symbols in the universe "
                         f"({len(symbols) - len(set(symbols))} duplicate(s)) -- "
                         "the subscribed set is not what the count claims")
...
async def build(args) -> int:
    ...
    manifest = {
        "schema": "gate1.universe_manifest/2",
        "session_id": args.session_id,
        "built_at": universe_doc["built_at"],
        "universe_file": str(universe_path),
        "universe_sha256": sha256_file(universe_path),
        # THE SYMBOLS, IDENTIFIED INDEPENDENTLY OF THE FILE THAT CARRIES THEM.
        #
        # universe_sha256 hashes universe.json as bytes, so it changes if a
        # key is reordered, indentation shifts, or any unrelated field is
        # added -- and it says nothing about WHICH SYMBOLS were subscribed
        # unless you still hold that exact file. The manifest could therefore
        # prove a file unaltered while being unable to state what was measured.
        #
        # This hashes the symbol SET itself: sorted, newline-joined, UTF-8. It
        # is stable across formatting and instrument ordering, and changes if
        # and only if the membership changes. Two runs that subscribed the same
        # instruments produce the same symbols_sha256 whatever their files look
        # like; two runs that did not, cannot.
        #
        # Sorted deliberately. Emission order is an artefact of how the builder
        # walks tiers, not a fact about the universe, and hashing it would make
        # an irrelevant reordering look like a different measurement.
        "symbols_sha256": _symbols_sha256(instruments),
        "symbols_sha256_method": ("sha256 of the sorted, newline-joined, "
                                  "UTF-8 symbol list (no trailing newline)"),
        "measured_symbol_count": len(instruments),
        "role": "PROVISIONAL" if args.provisional else "CANONICAL",
        "opening_buffer_points": (args.opening_buffer_points
                                  if args.provisional else 0),
