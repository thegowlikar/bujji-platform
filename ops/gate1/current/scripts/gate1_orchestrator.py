...
def main() -> int:
    ...
    # RECONNECT IS OPT-IN, mirroring --capacity-probe.
    #
    # It used to be passed to the harness UNCONDITIONALLY, hardcoded in the
    # Popen argument list with no flag able to suppress it. Capacity was
    # already conditional, so the two were inconsistent: capacity you asked
    # for, reconnect you got whether you wanted it or not.
    #
    # WHY THAT BREAKS CLEAN CAPTURE. gate1_opening_capture computes
    #   sustained_until = stop_at - reconnect_budget - probe_budget
    # so an always-on reconnect reserved 540s and ENDED CONTINUOUS CAPTURE
    # NINE MINUTES BEFORE THE CLOSE, on the busiest window of the day for
    # options. Reconnect is also subscription-changing by construction: it
    # drops the socket and resubscribes. A session claiming uninterrupted
    # 09:15-15:40 capture cannot contain one.
    #
    # Default OFF. Reconnect recovery and subscription capacity are real
    # measurements, but they belong to dedicated live-venue experiments with
    # their own scope and evidence claim -- not bolted onto a capture day.
    ap.add_argument("--reconnect-test", action="store_true",
                    help="run the reconnect-recovery phase. OFF by default: it "
                         "is subscription-changing and reserves budget out of "
                         "continuous capture.")
    ...
    try:
        ...
        log("MODE: "
            + ("clean capture -- continuous to the close; reconnect and "
               "capacity are UNMEASURED and deferred to dedicated experiments"
               if not (args.reconnect_test or args.capacity_probe)
               else f"instrumented -- reconnect={args.reconnect_test} "
                    f"capacity={args.capacity_probe}; continuous capture is "
                    f"interrupted before the close"))
        ...
        if args.dry_run in ("mock_pass", "notify_fail"):
            ...
            proc = subprocess.Popen(
                ...
                # NO FLAG AT ALL when disabled, not a flag with a zero budget.
                # Passing --reconnect-test with --reconnect-seconds 0 would
                # still admit a subscription-changing phase; the harness must
                # not be told to reconnect in the first place.
                + (["--reconnect-test",
                    "--reconnect-seconds", str(BUDGET["reconnect_s"])]
                   if args.reconnect_test else [])
