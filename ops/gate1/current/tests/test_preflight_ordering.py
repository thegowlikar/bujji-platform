...
try:
    ...
    if not (REAL_UNIVERSE / "universe.json").exists():
        ...
        # THE PAIR MUST BE REPAIRED WHEN COPIED, NOT JUST COPIED.
        #
        # This used to copy universe.json and its manifest into a DIFFERENT
        # session directory unchanged, leaving the manifest declaring a
        # universe_file it was no longer paired with. The v1 preflight never
        # noticed, so the fixture was quietly relying on that gap. v2 matches
        # the pair by resolved path, so the copy is now rehomed: universe_file
        # is repointed and the content hash recomputed, producing a legitimate
        # pair rather than a mismatched one that happens to pass.
        import hashlib as _h, json as _j
        shutil.copy(REAL_UNIVERSE / "universe.json", SESSION_DIR / "universe.json")
        _man = _j.loads((REAL_UNIVERSE / "universe_manifest.json").read_text())
        _man["universe_file"] = str(SESSION_DIR / "universe.json")
        _man["universe_sha256"] = _h.sha256(
            (SESSION_DIR / "universe.json").read_bytes()).hexdigest()
        (SESSION_DIR / "universe_manifest.json").write_text(_j.dumps(_man, indent=1))
