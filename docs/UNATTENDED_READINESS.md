...
| **API rate limits** | **Verified live** (not assumed): a real burst against the FYERS API produced `{'message': 'request limit reached', 'code': 429, 's': 'error'}`. Confirmed this is correctly **not** misclassified as an auth failure. **Found and fixed**: `get_recent_candles` previously only raised on `s/code`-classified auth errors; a rate-limited `historical` response had neither, so it fell through to `data.get("candles", [])`, returning an **empty list silently** instead of being retried. Fixed with a new `_raise_if_error()` classifier (`bujji/broker/fyers.py`) that raises on any non-auth FYERS error response, so `ExecutionEngine`'s existing retry/backoff now retries it — the same behavior `get_spot`/`get_ltp` already had via their own missing-key lookup. Transport-only change: no strategy or orchestration logic touched. Regression-tested (`test_get_recent_candles_raises_on_rate_limit_instead_of_silent_empty`, plus a test confirming auth-classified errors still raise `AuthenticationError` first, not the new generic error). |
