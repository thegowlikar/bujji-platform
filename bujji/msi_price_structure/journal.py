"""Price Structure Intelligence journal — append-only, deterministic
recorder for PriceStructureAssessments.
...
class PriceStructureJournal:
    """Append-only JSONL journal of PriceStructureAssessments. Never
    ...
    def record_assessment(self, assessment: PriceStructureAssessment) -> None:
        self._append({"schema_version": SCHEMA_VERSION, "kind": "PRICE_STRUCTURE_ASSESSMENT", "assessment": assessment_to_dict(assessment)})
    ...
    def read_assessments(self) -> List[PriceStructureAssessment]:
        return [
            assessment_from_dict(record["assessment"])
            for record in self.read_all()
            if record.get("kind") == "PRICE_STRUCTURE_ASSESSMENT"
        ]
