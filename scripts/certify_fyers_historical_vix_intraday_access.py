"""Phase 17H.9 — India VIX Intraday (5-min) historical access certification.

Mirrors `certify_fyers_historical_spot_intraday_access.py` exactly --
same new `access_method="direct_sdk_fyers_historical_intraday_rest"`,
same 100-day-bounded probes. Only the symbol/instrument and the
boundary-probe window differ: VIX intraday's real earliest date was
live-bisected in Phase 17H.8 at the SAME exact epoch as spot,
2017-07-17 -- this script reconfirms that live rather than trusting
...
Usage (manual, on the VPS, any time):
    ...
    PYTHONPATH=/opt/bujji/app python scripts/certify_fyers_historical_vix_intraday_access.py
"""
...
ARTIFACT_NAME = "fyers_india_vix_intraday_historical_certification"
...
async def main() -> int:
    ...
    log = logging.getLogger("certify_fyers_historical_vix_intraday")
    ...
    limitations.append(
        "VIX intraday depth (~2017-07-17) is dramatically shallower than VIX daily "
        "(~2008-04-17) -- a genuinely different, resolution-dependent boundary, not "
        "previously known before Phase 17H.8's audit. Same real epoch as spot intraday, "
        "suggesting a platform-wide intraday cutoff rather than an instrument-specific one."
    )
