"""Phase 17H.9 — NIFTY Futures (Continuous) Intraday (5-min) Historical Reality Ingestion.

Mirrors `ingest_nifty_futures_daily_historical.py`'s futures-identity
handling (instrument_identity="NIFTY_FUT_CONTINUOUS" fixed, never the
request symbol; source_symbol is whatever contract symbol was resolved
live via `InstrumentMaster.resolve_nearest_future()`; continuity_method
="fyers_cont_flag_1") combined with
`ingest_nifty_spot_intraday_historical.py`'s intraday mechanics
(100-day chunks, real per-candle timestamp via `epoch_to_ist()`,
resolution=RESOLUTION_FIVE_MINUTE, the new
"direct_sdk_fyers_historical_intraday_rest" access_method).

DEFAULT_START = 2018-01-01, bracketing the live-bisected real earliest
intraday date (~2018-01-02, Phase 17H.8/17H.9) -- bounded by the
continuous series' own existence, same depth as daily futures (unlike
...
Usage (manual, on the VPS, any time):
    ...
    PYTHONPATH=/opt/bujji/app python scripts/ingest_nifty_futures_intraday_historical.py
"""
...
FYERS_RESOLUTION = "5"
DEFAULT_START = datetime.date(2018, 1, 1)  # Brackets the live-bisected earliest real intraday date (~2018-01-02).

CHUNK_DAYS = 100

RAW_ARTIFACT_DIR = REPO_ROOT / "data" / "historical_reality" / "raw_artifacts" / "fyers" / "NIFTY_FUT_CONTINUOUS" / "intraday_5min"
...
async def run(*, start: datetime.date, end: datetime.date) -> int:
    ...
    log = logging.getLogger("ingest_nifty_futures_intraday_historical")
    ...
    if cert_status != reality_taxonomy.CERTIFIED_AVAILABLE:
        print(
            ...
            "Run scripts/certify_fyers_historical_futures_intraday_access.py first.",
            file=sys.stderr,
        )
        return 1
    ...
    for chunk_start, chunk_end in _chunks(start, end, CHUNK_DAYS):
        ...
        try:
            raw = await broker._call(
                "historical", symbol=request_symbol, resolution=FYERS_RESOLUTION, date_format="1",
