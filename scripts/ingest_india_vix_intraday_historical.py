"""Phase 17H.9 — India VIX Intraday (5-min) Historical Reality Ingestion.

Mirrors `ingest_nifty_spot_intraday_historical.py` exactly (same
100-day chunking, same real per-candle timestamp via `epoch_to_ist()`,
same new `access_method`) -- only the symbol/instrument type and the
default start date differ. VIX intraday's real earliest date was
live-bisected at the SAME exact epoch as spot (Phase 17H.8): 2017-07-17.
...
Usage (manual, on the VPS, any time):
    ...
    PYTHONPATH=/opt/bujji/app python scripts/ingest_india_vix_intraday_historical.py
"""
...
RAW_ARTIFACT_DIR = REPO_ROOT / "data" / "historical_reality" / "raw_artifacts" / "fyers" / "NSE_INDIAVIX-INDEX" / "intraday_5min"
...
async def run(*, start: datetime.date, end: datetime.date) -> int:
    ...
    log = logging.getLogger("ingest_india_vix_intraday_historical")
    ...
    if cert_status != reality_taxonomy.CERTIFIED_AVAILABLE:
        print(
            ...
            "Run scripts/certify_fyers_historical_vix_intraday_access.py first.",
            file=sys.stderr,
        )
        return 1
