"""Phase 17H.6 — India VIX historical access certification.

Mirrors `certify_fyers_historical_spot_access.py` exactly (same
discipline, same reasoning about the `direct_sdk_fyers_historical_rest`
access-method distinctness -- see that script's docstring and
docs/PHASE_17H3_HISTORICAL_CERTIFICATION_AND_INGESTION_CONTRACT.md
Part 1.1). Only the instrument/symbol and the boundary-probe years
differ: VIX's real earliest date was already live-bisected in Phase
17H.1 (2007=no_data, 2008=ok) -- this script reconfirms that live
rather than trusting the carried-over number.

DELIBERATELY NOT MARKET-HOURS GATED, same reasoning as the spot script.

Usage (manual, on the VPS, any time):
    cd /opt/bujji/app
    set -a && source /tmp/local_fyers.env && set +a
    PYTHONPATH=/opt/bujji/app python scripts/certify_fyers_historical_vix_access.py
"""
...
ARTIFACT_NAME = "fyers_india_vix_historical_certification"
...
async def main() -> int:
    ...
    log = logging.getLogger("certify_fyers_historical_vix")
    ...
    try:
        raw_2007 = await broker._call(
            "historical", symbol=SYMBOL, resolution="D", date_format="1",
            range_from="2007-01-01", range_to="2007-12-31", cont_flag="1",
        )
        raw_2008 = await broker._call(
            "historical", symbol=SYMBOL, resolution="D", date_format="1",
            range_from="2008-01-01", range_to="2008-12-31", cont_flag="1",
        )
        no_data_2007 = raw_2007.get("s") == "no_data"
        ok_2008 = raw_2008.get("s") == "ok" and bool(raw_2008.get("candles"))
        if no_data_2007 and ok_2008:
            boundary_status = "OK"
            earliest_confirmed = datetime.datetime.fromtimestamp(
                raw_2008["candles"][0][0], tz=datetime.timezone.utc
            ).date().isoformat()
        else:
            boundary_status = "MISMATCH"
            limitations.append(
                f"Boundary reconfirmation mismatch: 2007 s={raw_2007.get('s')}, "
                f"2008 s={raw_2008.get('s')} candles={len(raw_2008.get('candles', []))}."
            )
    except Exception as e:  # noqa: BLE001
    ...
    if recent_status != "OK":
        ...
        limitations.append(
            ...
            "but the ~2008 depth claim is not independently reconfirmed this run."
        )
    else:
        result = CERTIFIED
