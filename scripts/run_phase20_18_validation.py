"""Phase 20.18 real-artifact validation script -- reuses Phase 20.5's
own published real evidence verbatim, and the real Phase 20.17.1
RiskContextAssessment outcomes as inputs, exactly matching every prior
Cycle-1 phase's own validation script pattern.
"""
...
from bujji.execution_intelligence import (
    ...
    explain_execution_intent, explain_execution_result, explain_no_execution_intent, simulate_execution,
)
...
def _run_scenario(label, decision, risk_status, snapshot):
    ...
    risk = RiskContextAssessment(
        status=risk_status, risk_context_valid=risk_status not in (STATUS_NOT_EVALUATED,),
    ...
    if intent is None:
        print(explain_no_execution_intent(decision.strategy_name, decision.decision_state))
        print()
        return
    plan = build_execution_plan(intent, risk)
    print(explain_execution_intent(intent, plan))
    if not plan.simulation_required:
        print("Simulation NOT run (plan.simulation_required=False).")
        print()
        return
    result = simulate_execution(plan, snapshot, seed=7)
    quality = assess_execution_quality(result)
    print(explain_execution_result(result, quality))
    print()
...
def main():
    ...
    _run_scenario(
        ...
        MarketSnapshot(symbol="NIFTY", last_price=24500.0, volatility=120.0, liquidity_score=0.85),
    )
    ...
    _run_scenario(
        ...
        STATUS_RESTRICTED,
        MarketSnapshot(symbol="NIFTY", last_price=24500.0, volatility=500.0, liquidity_score=0.1),
    )
